import { Button, Card, Col, DatePicker, Input, InputNumber, Row, Select, Space, Statistic, Table, Typography } from "antd";
...
import {
	dailyUseCalculators,
	formatMoney,
	type IDailyInputDefinition,
	type IDailyInputValue,
	type IDailyMetric,
	type IDailyUseCalculatorId,
} from "./calculators";
...
const getMetricColor = (tone: IDailyMetric["tone"]) => {
	if (tone === "success") {
		return "#15803d";
	}
...
const DailyInput = ({
	...
	input: IDailyInputDefinition;
	value: IDailyInputValue;
	onChange: (value: IDailyInputValue) => void;
}) => {
	...
	if (input.type === "date" || input.type === "datetime-local") {
		const format = dateFormats[input.type];
		const pickerValue = String(value) ? dayjs(String(value), format) : null;

		return (
			<DatePicker
				showTime={input.type === "datetime-local" ? { format: "HH:mm" } : false}
				value={pickerValue?.isValid() ? pickerValue : null}
				onChange={(nextValue) => onChange(nextValue ? nextValue.format(format) : "")}
				format={input.type === "datetime-local" ? "YYYY-MM-DD HH:mm" : format}
				style={{ width: "100%" }}
			/>
		);
	}
	...
	if (input.type === "time") {
		return <Input type={input.type} value={String(value)} onChange={(event) => onChange(event.target.value)} />;
	}
...
export const DailyUseCalculator = ({ calculatorId }: { calculatorId: IDailyUseCalculatorId }) => {
	const calculator = dailyUseCalculators[calculatorId];
	const gradientIdPrefix = useId().replace(/:/g, "");
	const [values, setValues] = useState<Record<string, IDailyInputValue>>(calculator.defaults);
	const result = calculator.calculate(values);
	const areaDataKeys = Object.keys(result.areaData?.[0] ?? {}).filter((key) => key !== "name");

	const updateValue = (key: string, value: IDailyInputValue) => {
							...
							<Space orientation="vertical" size="middle" style={{ width: "100%" }}>
								{calculator.inputs.map((input) => (
									<Space orientation="vertical" size={4} style={{ width: "100%" }} key={input.key}>
										...
										<DailyInput input={input} value={values[input.key]} onChange={(value) => updateValue(input.key, value)} />
									</Space>
								))}
							</Space>
						</Card>
						...
						<Space orientation="vertical" size="large" style={{ width: "100%" }}>
							...
							<Row gutter={[16, 16]}>
								...
								{result.barData?.length ? (
									<Col xs={24} xl={result.radialData?.length ? 12 : 24}>
								...
								{result.radialData?.length ? (
									<Col xs={24} xl={result.barData?.length ? 12 : 24}>
										<Card title="Progress">
											<ChartFrame>
												{(width) => (
													<RadialBarChart
													...
													>
														...
														<RadialBar dataKey="value" fill={chartColors[1]} background cornerRadius={8} />
